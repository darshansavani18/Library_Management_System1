...
def open_receive_book(root):
    win = tk.Toplevel(root)
    win.title("Receive Book")
    win.state("zoomed")
    ...
    receive_date = tk.StringVar(value=datetime.date.today().strftime("%Y-%m-%d"))
    sr_no = tk.StringVar()
    ...
    h_field("Return Date", receive_date, 0)
    ...
    tk.Label(right, text="Currently Issued Books",
             font=("Segoe UI", 14, "bold"), bg="white").pack(pady=8)
    ...
    def receive_book():
        ...
        conn.close()
        load_issued()
        messagebox.showinfo("Success", "Book Received Successfully")

    tk.Button(
        win, text="RECEIVE BOOK",
        bg="#27ae60", fg="white",
        font=("Segoe UI", 12, "bold"),
        command=receive_book
    ).pack(pady=10)
